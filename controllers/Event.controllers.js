  ...
  if (hostIds.length > 0) {
    ...
    for (const h of hosts) {
      ...
      hostMap[h.id] = {
        ...
        status: h.status,
        user_id: h.user_id,
        phone: h.phone || h.whatsapp || u?.phone || "",
        email: h.email || u?.email || "",
        whatsapp: h.whatsapp || h.phone || u?.phone || "",
        country: h.country,
        state: h.state,
        city: h.city,
        facebook: h.facebook || "",
        instagram: h.instagram || "",
        selfie_photo: h.selfie_photo || "",
...
export const createEventDraft = async (req, res) => {
  try {
    ...
    if (!host) {
      return res.status(403).json({ success: false, message: "Your host account is pending admin approval. You can create events once your application is approved." });
    }
...
export const getApprovedEvents = async (req, res) => {
  try {
    ...
    const country = req.query.country || req.headers["x-country"] || undefined;
    ...
    if (country) {
      ...
      result = result.filter(e => !e.country || e.country.toLowerCase().trim() === countryLower || countryLower.includes(e.country?.toLowerCase()?.trim()));
    }
...
export const getEventById = async (req, res) => {
  try {
    ...
    const hostUserId = eventData.Host?.user_id || eventData.host_user_id || null;
    const isOwner = hostUserId && String(hostUserId) === String(requesterId);
    const hasContactAccess = isOwner || (requesterId && hostUserId ? await checkContactAccess({ requesterId, targetUserId: hostUserId, itemId: eventData.id }) : false);
    const sanitizedEventData = JSON.parse(JSON.stringify(eventData));
    if (sanitizedEventData.Host) {
      if (sanitizedEventData.Host) {
        if (hasContactAccess) {
          if (!sanitizedEventData.Host.phone || !sanitizedEventData.Host.email || !sanitizedEventData.Host.whatsapp) {
            try {
              const hostDoc = await Host.get(sanitizedEventData.Host.id || eventData.host_id);
              const userDoc = hostUserId ? await User.get(hostUserId) : null;
              sanitizedEventData.Host.phone = sanitizedEventData.Host.phone || hostDoc?.phone || hostDoc?.whatsapp || userDoc?.phone || "";
              sanitizedEventData.Host.email = sanitizedEventData.Host.email || hostDoc?.email || userDoc?.email || "";
              sanitizedEventData.Host.whatsapp = sanitizedEventData.Host.whatsapp || hostDoc?.whatsapp || hostDoc?.phone || userDoc?.phone || "";
              sanitizedEventData.Host.facebook = sanitizedEventData.Host.facebook || hostDoc?.facebook || "";
              sanitizedEventData.Host.instagram = sanitizedEventData.Host.instagram || hostDoc?.instagram || "";
            } catch (e) { }
          }
        } else {
          sanitizedEventData.Host.phone = "";
          sanitizedEventData.Host.email = "";
          sanitizedEventData.Host.whatsapp = "";
          sanitizedEventData.Host.facebook = "";
          sanitizedEventData.Host.instagram = "";
        }
      }
    }
    return res.json({ success: true, event: sanitizedEventData, is_registered: isRegistered });
  } catch (err) {
    return res.status(500).json({ message: "Server error" });
  }
};
