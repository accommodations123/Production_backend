...
export const getBuySellListingById = async (req, res) => {
    try {
        ...
        if (!listing || (listing.status !== "active" && listing.status !== "approved")) {
        ...
        // Fetch seller's host profile for additional contact details with access authorization
        const requesterId = req.user?.id || null;
        const isOwner = String(processedListing.user_id) === String(requesterId);
        const hasContactAccess = isOwner || (requesterId ? await checkContactAccess({ requesterId, targetUserId: processedListing.user_id, itemId: processedListing.id }) : false);

        try {
            const hosts = await Host.query("user_id").eq(processedListing.user_id).exec();
            const host = hosts?.[0];
            if (host) {
                processedListing.sellerInstagram = hasContactAccess ? (host.instagram || "") : "";
                processedListing.sellerFacebook = hasContactAccess ? (host.facebook || "") : "";
                processedListing.sellerEmail = hasContactAccess ? (host.email || processedListing.email || "") : "";
                processedListing.sellerPhone = hasContactAccess ? (host.phone || processedListing.phone || "") : "";
                processedListing.sellerWhatsapp = hasContactAccess ? (host.whatsapp || "") : "";
            } else {
                processedListing.sellerInstagram = "";
                processedListing.sellerFacebook = "";
                processedListing.sellerEmail = hasContactAccess ? (processedListing.email || "") : "";
                processedListing.sellerPhone = hasContactAccess ? (processedListing.phone || "") : "";
                processedListing.sellerWhatsapp = "";
            }
        } catch (hostErr) {
...
export const getAdminApprovedBuySellListings = async (req, res) => {
    try {
        ...
        const processedListings = await Promise.all(listings.map(async listing => {
            ...
            // Sanitize public seller contact details for listing items
            l.sellerInstagram = "";
            l.sellerFacebook = "";
            l.sellerEmail = "";
            l.sellerPhone = "";
