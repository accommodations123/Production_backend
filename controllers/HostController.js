...
import { batchGetUsers } from "../utils/batchUtils.js";
...
export const getPendingHosts = async (req, res) => {
  try {
    ...
    // Fetch user data for each host in batch
    const userIds = results.map(h => h.user_id).filter(Boolean);
    const users = await batchGetUsers(userIds);
    const userMap = new Map(users.map(u => [u.id, u]));

    const processedHosts = results.map((host) => {
