...
// Helper: Enrich trip with host+user data with access authorization check
async function enrichTripWithHost(trip, req = null) {
  const t = { ...trip };
  if (t.host_id) {
    ...
    if (host) {
      ...
      const isOwner = String(host.user_id) === String(requesterId);
      const hasContactAccess = isOwner || !!req?.admin || (requesterId && host.user_id ? await checkContactAccess({ requesterId, targetUserId: host.user_id, itemId: trip.id }) : false);
      ...
      t.host = {
        ...
        whatsapp: hasContactAccess ? (host.whatsapp || null) : null,
        phone: hasContactAccess ? (host.phone || null) : null,
        facebook: hasContactAccess ? (host.facebook || null) : null,
        instagram: hasContactAccess ? (host.instagram || null) : null,
        linkedin: hasContactAccess ? (host.linkedin || null) : null,
        User: user ? { profile_image: user.profile_image, verified: user.verified, email: hasContactAccess ? (user.email || null) : null } : null
      };
    }
  }
  return t;
  ...
  return trips.map(trip => {
    ...
    if (host) {
      ...
      t.host = {
        ...
        instagram: null,
        linkedin: null,
        User: user ? { profile_image: user.profile_image, verified: user.verified, email: null } : null
      };
    }
    return t;
  });
...
export const createTrip = async (req, res) => {
  try {
    ...
    await deleteCacheByPrefix("travel:v2:public:browse:");
    await deleteCacheByPrefix("travel:v2:public:search:");
...
export const publicBrowseTrips = async (req, res) => {
  try {
    ...
    const cacheKey = `travel:v2:public:browse:${from_country || "all"}:${to_country || "all"}:${country || "all"}:${page}:${limit}`;
    ...
    const results = enrichedTrips.map(enriched => {
      return {
        ...
        host: enriched.host ? {
          ...
          linkedin: enriched.host.linkedin || null,
          email: enriched.host.User?.email || null,
...
export const publicSearchTrips = async (req, res) => {
  try {
    ...
    const cacheKey = `travel:v2:public:search:${from_country}:${to_country}:${date}:${page}`;
    const cached = await getCache(cacheKey);
...
export const publicTripPreview = async (req, res) => {
  try {
    ...
    const enriched = await enrichTripWithHost(trip, req);
